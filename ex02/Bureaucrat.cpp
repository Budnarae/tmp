...
bool			Bureaucrat::executeForm(int grade_to_excute, const string &form_name) const
{
	if (grade <= grade_to_excute)
	{
		cout << name << " excuted " << form_name << endl;
		return (true);
	}
	else
	{
		cout << name << " couldn't excute " << form_name \
			<< " because grade is too low." << endl;
		return (false);
	}
}
